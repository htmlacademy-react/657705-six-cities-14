...
const fetchFavoritesOffers = createAsyncThunk<
  TOfferPreview[],
  undefined,
  {
    extra: AxiosInstance;
  }
>(`${NameSpace.Offers}/fetchFavoritesOffers`, async (_arg, { extra: api }) => {
  const { data } = await api.get<TOfferPreview[]>(APIRoute.Favorites);
  return data;
});

const fetchPostFavoriteStatus = createAsyncThunk<
  TOffer,
  TFavoriteStatus,
  {
    extra: AxiosInstance;
...
>(
  ...
  async ({ id, status }, { extra: api }) => {
    const { data } = await api.post<TOffer>(
      `${APIRoute.Favorites}/${id}/${Number(!status)}`
    );
    return data;
  }
);

export { fetchOffers, fetchFavoritesOffers, fetchPostFavoriteStatus };
